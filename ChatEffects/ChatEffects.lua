...
local function ChatPrefixFilter(self, event, msg, author, ...)
    ...
    -- Split message into link vs non-link segments
    local segments = {}
    local lastEnd = 1
    for startPos, endPos in msg:gmatch("()|c%x%x%x%x%x%x%x%x|H.-|h.-|h|r()") do
        if startPos > lastEnd then
            segments[#segments+1] = { text = msg:sub(lastEnd, startPos - 1), isLink = false }
        end
        segments[#segments+1] = { text = msg:sub(startPos, endPos - 1), isLink = true }
        lastEnd = endPos
    end
    if lastEnd <= #msg then
        segments[#segments+1] = { text = msg:sub(lastEnd), isLink = false }
    end
    ...
    -- STATIC COLORS (case-insensitive)
    local colorName = lowerMsg:match("^([a-z]+):")
    if colorName and staticColors[colorName] then
        local rgb = staticColors[colorName]
        local out, stripped = {}, false
        ...
        for _, seg in ipairs(segments) do
            if seg.isLink then
                ...
                if not stripped then
                    txt = txt:gsub("^[Cc][Yy][Aa][Nn]:%s*", ""):gsub("^"..colorName..":%s*", "")
                    stripped = true
                end
                out[#out+1] = string.format("|cff%02x%02x%02x%s|r",
                    rgb[1]*255, rgb[2]*255, rgb[3]*255, txt)
            end
        end
    ...
    -- RAINBOW (case-insensitive)
    if lowerMsg:match("^rainbow:") then
        local out, stripped = {}, false
        for _, seg in ipairs(segments) do
            if seg.isLink then
                out[#out+1] = seg.text
                ...
                local txt = seg.text
                if not stripped then
                    txt = txt:gsub("^[Rr][Aa][Ii][Nn][Bb][Oo][Ww]:%s*", "")
                    stripped = true
                end
                out[#out+1] = BuildRainbow(txt)
            end
        end
        return false, table.concat(out), author, ...
    end
    ...
    local pat = lowerMsg:match("^pattern([0-9a-z]+):")
    if pat then
        local out, stripped = {}, false
        for _, seg in ipairs(segments) do
            if seg.isLink then
                ...
                local txt = seg.text
                if not stripped then
                    txt = txt:gsub("^pattern"..pat..":%s*", "")
                    stripped = true
                end
                out[#out+1] = BuildPattern(txt, pat)
            end
        end
        return false, table.concat(out), author, ...
    end

    -- GLOW/FLASH (strip prefix)
    local style, clean2 = lowerMsg:match("^([a-z0-9_]+):%s*(.+)")
    if style and (glowColours[style] or flashColours[style]) then
        return false, clean2, author, ...
    end
